...
const Select = React.forwardRef<HTMLSelectElement, SelectProps>(
  ({ className, children, ...props }, ref) => {
    return (
      <select
        className={cn(
          'flex h-11 w-full appearance-none rounded-xl border border-border bg-card bg-[length:1rem] bg-[right_1rem_center] bg-no-repeat px-4 py-2.5 pr-10 text-sm text-foreground shadow-soft transition-colors',
